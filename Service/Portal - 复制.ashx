...
public class Portal : IHttpHandler, IRequiresSessionState
{
    ...
    public void BaseUser_GetOutlayInfo()
    {
        ...
        sql.Append("convert(varchar(10),audittime,23) as thisdate from SpecialOutlayApplyDetail where deptid=@deptid and status=4 and datediff(dd,audittime,getdate())<=5 ");
    ...
    public void Auditor_GetUnAuditApproveOutlay()
    {
        ...
        sql.Append("select  convert(varchar(4),datepart(yyyy,approvetime))+'年'+");
        sql.Append("convert(varchar(2),datepart(MM,approvetime))+'月'+convert(varchar(2),datepart(dd,approvetime))+'日'+");
        sql.Append("approver+'批转'+deptname+'的追加经费申请,请处理' as title ");
        sql.Append(" from SpecialOutlayApplyDetail where status=3 " + scopeWhere);
    ...
    public void Director_GetOutlayUnApprove()
    {
        ...
        sql.Append(" from  SpecialOutlayApplyDetail where status=1 " + scopeWhere + " union ");
        //直接追加
        sql.Append("select  convert(varchar(4),datepart(yyyy,applytime))+'年'+");
        sql.Append("convert(varchar(2),datepart(MM,applytime))+'月'+convert(varchar(2),datepart(dd,applytime))+'日'+");
