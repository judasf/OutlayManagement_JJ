...
public class SpecialOutlayAllocate : IHttpHandler, IRequiresSessionState
{
    ...
    public string SetQueryConditionForApplyOutlayDetail()
    {
        ...
        {
            if ((roleid == "1" || roleid == "6") && Request.Form["status"].ToString() == "1")//基层用户和管理员查询待审批信息时包含处长未审批(1)的和被稽核退回给处长的(2)
                list.Add(" status =1 or status=2 ");
            else
        ...
        //处长默认显示已送审(1)和被稽核退回(2)的信息；
        //稽核默认显示已审批未确认(3)可查询已确认生成(4)的信息;
        //管理员默认显示已生成(4)的信息可查询全部信息
    ...
    public void SaveSpecialOutlayApplyDetail()
    {
        ...
        paras[5].Value = DateTime.Now.ToString("yyyy年MM月");
        string sql = "INSERT INTO SpecialOutlayApplyDetail(ApplyTime,DeptId,DeptName,ApplyTitle,ApplyContent,Status,ApplyUser,OutlayMonth) VALUES(getdate(),@deptid,@deptname,@title,@content,'0',@applyuser,@outlaymonth)";
    ...
    public void UpdateSpecialOutlayApplyDetail()
    {
        ...
        string sql = "update  SpecialOutlayApplyDetail set ApplyTitle=@title,ApplyContent=@content where id=@id";
        int result = SqlHelper.ExecuteNonQuery(SqlHelper.GetConnection(), CommandType.Text, sql, paras);
        if (result == 1)
            Response.Write("{\"success\":true,\"msg\":\"执行成功\"}");
        else
