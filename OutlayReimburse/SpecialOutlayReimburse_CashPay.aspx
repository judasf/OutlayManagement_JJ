...
<script type="text/javascript">
    ...
    var showCashDetail = function (id) {
        var dialog = parent.$.modalDialog({
            ...
            href: 'OutlayReimburse/dialogop/CashReimDetail_op.aspx?id=' + id, //将对话框内容添加到父页面index
            buttons: [{
                text: '关闭',
                handler: function () {
                    dialog.dialog('close');
    ...
    var exportCashReimburse = function () {
        jsPostForm('../service/ReimburseOutlay.ashx/ExportUserSpecialCashReimburse?type=2', $.serializeObject($('#spCashSearch')));
    };
    //现金支出表
    var cashGrid;
    $(function () {
        ...
        cashGrid = $('#cashGrid').datagrid({
            ...
            noheader: true,
            showFooter: true,
            url: '../service/ReimburseOutlay.ashx/GetCashPay?type=2',
            striped: true,
            rownumbers: true,
            pagination: true,
            pageSize: 20,
...
<div id="cgTip">
    <form id="spCashSearch" style="margin: 0;">
    <table>
        <tr>
            ...
            },onSelect:function(node){if(node) $('#spCashSearch').find('#outlayCategory').val(node.text);}" />
            </td>
            <td width="50" align="right">
                审核：
            </td>
            ...
            <td>
                <input id="finishstatus" class="easyui-combobox"
                    data-options="panelHeight:'auto',editable:false,valueField:'value',textField:'text',data:[{'value':'0','text':'待受理'},{'value':'2','text':'已办结'}],onSelect:function(rec){(rec.value>0)&&$('#spCashSearch').find('#status').combobox('setValue','3');}"
            ...
            <td>
                <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-magnifier',plain:true"
                    onclick="cashGrid.datagrid('load', $.serializeObject($('#spCashSearch')));">查询</a>
                <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-magifier_zoom_out',plain:true"
                    onclick="  $('#spCashSearch input').val('');cashGrid.datagrid('load', {});">重置</a>
