...
<script type="text/javascript">
    ...
    var exportCardReimburse = function () {
        jsPostForm('../service/ReimburseOutlay.ashx/ExportUserPublicCardReimburse?type=1', $.serializeObject($('#pcsearchForm')));
    };
    //公务卡支出表
    var cardGrid;
    $(function () {
        ...
        cardGrid = $('#cardGrid').datagrid({
            title: '公务卡支出明细',
            url: '../service/ReimburseOutlay.ashx/GetCardPay?type=1',
            striped: true,
            rownumbers: true,
            pagination: true,
            noheader: true,
            ...
            onLoadSuccess: function (data) {
                ...
                //提示框
                $(this).datagrid('tooltip', ['memo', 'cardnumber', 'spendingtime', 'auditorcomment']);
            }
        });
        //设置分页属性
...
<div id="cardTip">
    <form id="pcsearchForm" style="margin: 0;">
        <table>
            <tr>
                ...
                <td>
                    <input id="finishstatus" class="easyui-combobox" data-options="panelHeight:'auto',editable:false,valueField:'value',textField:'text',data:[{'value':'0','text':'待受理'},{'value':'2','text':'已办结'}],onSelect:function(rec){(rec.value>0)&&$('#pcsearchForm').find('#status').combobox('setValue','3');}" name="finishstatus" style="width: 60px" />
                </td>
                <td>
                    <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-magnifier',plain:true"
                        onclick="cardGrid.datagrid('load', $.serializeObject($('#pcsearchForm')));">查询</a>
                    <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-magifier_zoom_out',plain:true"
                        onclick="  $('#pcsearchForm input').val('');cardGrid.datagrid('load', {});">重置</a>
    ...
    <div style="background: url(../js/easyui/themes/icons/tip.png) no-repeat 10px 5px; line-height: 24px; padding-left: 30px;">
        公务卡支出明细无需取回被退回的金额，当支出申请被稽核退回时额度自动回复
    </div>
</div>
