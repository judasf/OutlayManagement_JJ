<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AuditAccountReimburse.aspx.cs"
    Inherits="OutlayReimburse_AuditAccountReimburse" %>
...
<head>
    ...
    <script type="text/javascript">
        //转账支出处理对话框
        var showAccountDetail = function (id, status) {
            var btns = [{
                text: '关闭',
                handler: function () {
                    dialog.dialog('close');
            ...
            if (roleid == 2) {
                ...
                if (status == 1)
                    btns.unshift({
                        text: '审核',
                        handler: function () {
                            parent.onFormSubmit(dialog, accountGrid);
                        }
                    }, {
                        text: '退回',
                        handler: function () {
                            parent.onFormBack(dialog, accountGrid);
                        }
                    })
                if (status == 2)
                    btns.unshift({
            ...
            var dialog = parent.$.modalDialog({
                ...
                href: 'OutlayReimburse/dialogop/AccountReimDetail_op.aspx?id=' + id,
                buttons: btns
            });
        };
        //查询功能
        ...
        var exportAccountReimburse = function () {
            jsPostForm('../service/ReimburseOutlay.ashx/ExportAuditAccountReimburse', $.serializeObject($('#searchForm')));
        };
        //转账支出表
        var accountGrid;
        $(function () {
                ...
                frozenColumns: [[
            {
              ...
              [{
                ...
                onLoadSuccess: function (data) {
                    ...
                    //提示框
                    $(this).datagrid('tooltip', ['outlaycategory', 'memo', 'payeename', 'accountnumber', 'bankname']);
                },
                onDblClickRow: function (index, row) {
                    showAccountDetail(row.id, row.status);
                }
            });
            //设置分页属性
